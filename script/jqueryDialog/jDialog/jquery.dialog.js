  ...
  DialogClass.prototype = {
    ...
    title: function (title) {
      if ((title || "").length) {
        // setter
        this.dom.element
          .find(".j-dialog-title>span.j-dialog-txt")
          .html(title || "");
        return this;
      } else {
        // getter
    ...
    middle: function () {
      ...
      var win = $(window);
      var o =
        this.cfg.fixed && !isIE6 ? [0, 0] : [doc.scrollLeft(), doc.scrollTop()];
    ...
    position: function (pos) {
      if (
        (!pos.left || isNaN(parseInt(pos.left, 10))) &&
        (!pos.top || isNaN(parseInt(pos.top, 10)))
      ) {
        // getter
    ...
    buttons: function (buttons) {
      ...
      if (buttons && buttons.length > 0) {
        ...
        $.each(buttons, function (i, btn) {
          ...
          v = btn.text || "确定";
          htmlBtns.push(
            '<input type="button" class="' + cls + '" value="' + v + '" />'
          );
        });
        var btnContainer = $('<div class="j-dialog-buttons"></div>')
    ...
    _init: function () {
      ...
      if (
        !isNaN(parseInt(this.cfg.top)) ||
        !isNaN(parseInt(this.cfg.left)) ||
        this.cfg.anchor
      ) {
      ...
      var className =
        "j-dialog " +
        (this.cfg.dialogClassName ? this.cfg.dialogClassName : "");
      // 如果配置了fixed
      ...
      // dialog节点
      this.dom.element = $(
        '<div class="' + className + '" id="' + this.cfg.divid + '"></div>'
      )
        .css({
      ...
      if (this.cfg.anchor) {
        ...
        if (!this._tempVars.domAnchor[0]) {
          throw new Error(
            'The "anchor.target" must be a HTMLElement instance!'
          );
          return this;
        }
    ...
    _setupTitleBar: function () {
      ...
      if (this.cfg.closeable) {
        var btnClose = $(
          '<a href="#" class="j-dialog-close" title="关闭">&nbsp;</a>'
        )
          .appendTo(this.dom.element)
          .bind({
            click: function (evt) {
      ...
      // titleBar 设置
      this._tempVars.titleBar = $(
        '<div class="j-dialog-title"><span class="j-dialog-txt"></span></div>'
      ).appendTo(this.dom.element);
    ...
    _setupHackDiv: function (display) {
      ...
      if (display) {
        if ($("IFRAME", self.dom.element).length > 0) {
          ...
          self.hack_div = (self.hack_div || $("<div></div>"))
            .appendTo(con)
            .css({
              position: "absolute",
              left: 0,
      ...
      switch ((this.cfg.anchor.position || "right").toLowerCase()) {
        ...
        case "top":
          ...
          posDialog = {
            ...
            left:
              domAnchorOffset.left +
              (domAnchorSize.width - domDialogSize.width) / 2 +
              offset.left,
          };
          break;
        ...
        case "top-right":
          posTriangle = {
            top: 0,
            left: tpfs
              ? domDialogSize.width - 24 - tpfs
              : (domDialogSize.width - 24) / 2,
          };
          posDialog = {
            top: domAnchorOffset.top - domDialogSize.height - 12 - offset.top,
            left:
        ...
        case "right":
          posTriangle = {
            top: tpfs
              ? tpfs - domDialogSize.height
              : -(domDialogSize.height + 24) / 2,
            left: -24,
          };
          posDialog = {
            top:
              domAnchorOffset.top +
              (domAnchorSize.height - domDialogSize.height) / 2 +
              offset.top,
            left:
              domAnchorOffset.left + domAnchorSize.width + 12 + offset.right,
          };
          break;
        ...
        case "right-bottom":
          ...
          posDialog = {
            top:
              domAnchorOffset.top +
              domAnchorSize.height -
              domDialogSize.height -
              offset.bottom,
            left:
              domAnchorOffset.left + domAnchorSize.width + 12 + offset.right,
          };
        ...
        case "bottom":
          ...
          posDialog = {
            top:
              domAnchorOffset.top + domAnchorSize.height + 12 + offset.bottom,
            left:
              domAnchorOffset.left +
              (domAnchorSize.width - domDialogSize.width) / 2 +
              offset.left,
        ...
        $.each(["es"], function (k, v) {
          ...
          jDrag.init({
            ...
            onMove: function (event, x, y) {
              ...
              if (
                !(
                  (self.cfg.minWidth && width < self.cfg.minWidth && x < 0) ||
              ...
              if (self.hack_div) self.hack_div.css({ width: w, height: h });
            },
            onUp: function (event) {
              self._setupHackDiv(0);
              self.dom.element.removeClass("j-user-select");
